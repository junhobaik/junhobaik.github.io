...
class tagListTemplate extends Component {
  ...
  handlePlusClick = () => {
    const { tagShowCnt } = this.state;

    if (this.mobileDetect()) {
      this.setState({
        tagShowCnt: tagShowCnt + 6,
      });
    } else {
      this.setState({
        tagShowCnt: tagShowCnt + 15,
      });
    }
  };
  ...
  render() {
    ...
    // console.log(tagList, tagList.length,  this.state.tagShowCnt)
    return (
      <Layout location={location}>
        <div className="tag-wrap">
          <div className="tag-list">{tagList}</div>
          {tagList.length - 1 > this.state.tagShowCnt ? (
            <div className="tag-load" onClick={this.handlePlusClick}>
              + 더 보기
            </div>
          ) : (
            <div className="tag-load" onClick={this.handleMinusClick}>
              - 줄이기
            </div>
          )}
        </div>
